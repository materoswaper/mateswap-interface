...
describe('application reducer', () => {
  ...
  describe('updateBlockNumber', () => {
    it('updates block number', () => {
      store.dispatch(updateBlockNumber({ chainId: ChainId.LACCHAIN, blockNumber: 4 }))
      expect(store.getState().blockNumber[ChainId.LACCHAIN]).toEqual(4)
    })
    it('no op if late', () => {
      store.dispatch(updateBlockNumber({ chainId: ChainId.LACCHAIN, blockNumber: 2 }))
      expect(store.getState().blockNumber[ChainId.LACCHAIN]).toEqual(3)
    })
    it('works with non-set chains', () => {
      store.dispatch(updateBlockNumber({ chainId: ChainId.LACCHAIN_TESTNET, blockNumber: 2 }))
      expect(store.getState().blockNumber).toEqual({
        [ChainId.LACCHAIN]: 3,
        [ChainId.LACCHAIN_TESTNET]: 2,
      })
    })
  })

...
describe('transaction reducer', () => {
  ...
  describe('addTransaction', () => {
    it('adds the transaction', () => {
      ...
      expect(txs[ChainId.LACCHAIN]).toBeTruthy()
      expect(txs[ChainId.LACCHAIN]?.['0x0']).toBeTruthy()
      const tx = txs[ChainId.LACCHAIN]?.['0x0']
  ...
  describe('finalizeTransaction', () => {
    it('no op if not valid transaction', () => {
      store.dispatch(
        finalizeTransaction({
          chainId: ChainId.LACCHAIN_TESTNET,
          hash: '0x0',
          receipt: {
            status: 1,
            transactionIndex: 1,
    ...
    it('sets receipt', () => {
      ...
      const tx = store.getState()[ChainId.LACCHAIN_TESTNET]?.['0x0']
      expect(tx?.summary).toEqual('hello world')
  ...
  describe('clearAllTransactions', () => {
    it('removes all transactions for the chain', () => {
      ...
      expect(Object.keys(store.getState())).toEqual([String(ChainId.LACCHAIN), String(ChainId.LACCHAIN_TESTNET)])
      expect(Object.keys(store.getState()[ChainId.LACCHAIN] ?? {})).toEqual(['0x0'])
      expect(Object.keys(store.getState()[ChainId.LACCHAIN_TESTNET] ?? {})).toEqual(['0x1'])
      store.dispatch(clearAllTransactions({ chainId: ChainId.LACCHAIN }))
      expect(Object.keys(store.getState())).toHaveLength(2)
      expect(Object.keys(store.getState())).toEqual([String(ChainId.LACCHAIN), String(ChainId.LACCHAIN_TESTNET)])
      expect(Object.keys(store.getState()[ChainId.LACCHAIN] ?? {})).toEqual([])
      expect(Object.keys(store.getState()[ChainId.LACCHAIN_TESTNET] ?? {})).toEqual(['0x1'])
    })
  })
})

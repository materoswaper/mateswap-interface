...
export const MASTERCHEF_V2 = {
  [ChainId.LACCHAIN]: 'boneswap/master-chefv2',
}

export const masterChefV2 = async (query, chainId = ChainId.LACCHAIN, variables = undefined) =>
  request(`${GRAPH_HOST[chainId]}/subgraphs/name/${MASTERCHEF_V2[chainId]}`, query, variables)

export const MASTERCHEF_V1 = {
  [ChainId.LACCHAIN]: 'boneswap/master-chef',
}

export const masterChefV1 = async (query, chainId = ChainId.LACCHAIN, variables = undefined) =>
...
export const getMasterChefV2Farms = async (variables = undefined) => {
  ...
  const tokens = await getTokenSubset(ChainId.LACCHAIN, {

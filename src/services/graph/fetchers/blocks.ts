...
export const BLOCKS = {
  [ChainId.LACCHAIN]: 'blocklytics/ethereum-blocks',
}

export const fetcher = async (chainId = ChainId.LACCHAIN, query, variables = undefined) => {
  return request(`${GRAPH_HOST[chainId]}/subgraphs/name/${BLOCKS[chainId]}`, query, variables)
}

export const getBlock = async (chainId = ChainId.LACCHAIN, timestamp: number) => {
...
export const getBlocks = async (chainId = ChainId.LACCHAIN, start, end) => {
...
export const getMassBlocks = async (chainId = ChainId.LACCHAIN, timestamps) => {
...
export const getAverageBlockTime = async (chainId = ChainId.LACCHAIN) => {

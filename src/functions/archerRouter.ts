...
export abstract class ArcherRouter {
  ...
  public static swapCallParameters(
    ...
    const etherIn = trade.inputAmount.currency === LaChain.onChain(ChainId.LACCHAIN)
    const etherOut = trade.outputAmount.currency === LaChain.onChain(ChainId.LACCHAIN)
    // the router does not support both ether in and out
    invariant(!(etherIn && etherOut), 'ETHER_IN_OUT')
    invariant(!('ttl' in options) || options.ttl > 0, 'TTL')
    invariant('ethTip' in options && options.ethTip?.currency === LaChain.onChain(ChainId.LACCHAIN))
